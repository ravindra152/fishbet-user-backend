...
export class GenerateSpinHandler extends BaseHandler {
  async run () {
    ...
    if (checkFaucetAvailability?.isAvailable === false) { return checkFaucetAvailability }
    ...
    if (wheelConfiguration) {
      ...
      if (object?.sc > 0) { await TransactionHandlerHandler.execute({ userId, amount: object.sc, currencyCode: COINS.SWEEP_COIN.BONUS_SWEEP_COIN, purpose: TRANSACTION_PURPOSE.WHEEL_REWARD }, this.context) }

      if (object?.gc > 0) { await TransactionHandlerHandler.execute({ userId, amount: object.gc, currencyCode: COINS.GOLD_COIN, purpose: TRANSACTION_PURPOSE.WHEEL_REWARD }, this.context) }
